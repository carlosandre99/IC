...
[out_extraction_mod_nao_ideal, out_validation_mod_nao_ideal] = modulacao_demodulacao(in_extraction, out_validation, fc_mod, fc_demod, fs1, fs2, alfa, phi);
...
nmse_nao_ideal = 10 * log10(sum(abs(in_nao_ideal - out_extraction_mod_nao_ideal).^2) / sum(abs(in_nao_ideal).^2));
disp(['NMSE para o caso não ideal (fc_demod = ', num2str(fc_demod/1e6), ' MHz): ', num2str(nmse_nao_ideal), ' dB']);

figure;
espectro_frequencia_2(in_nao_ideal, fs1, 'Entrada do modulador', 'b', 'LineWidth', 2.5); 
hold on;
espectro_frequencia_2(out_extraction_mod_nao_ideal, fs1, 'Saída do demodulador', 'r', 'LineWidth', 1); 
%espectro_frequencia_2(saida_estimada_nao_ideal, fs1, 'Saída estimada', 'g', 'LineWidth', 1); 
hold off;
...
subplot(2,1,1);
hold on;
plot(t(1:150), real(in_nao_ideal(1:150)), 'b-', 'LineWidth', 1.5 , 'MarkerSize', 5, 'DisplayName', 'Entrada do modulador');
plot(t(1:150), real(out_extraction_mod_nao_ideal(1:150)), 'ro', 'LineWidth', 1.5, 'MarkerSize', 5, 'DisplayName', 'Saída do demodulador');
hold off;
xlabel('Tempo (s)');
ylabel('Parte real');
...
subplot(2,1,2);
hold on;
plot(t(1:150), imag(in_nao_ideal(1:150)), 'b-', 'LineWidth', 1.5 , 'MarkerSize', 5, 'DisplayName', 'Entrada do modulador');
plot(t(1:150), imag(out_extraction_mod_nao_ideal(1:150)), 'ro', 'LineWidth', 1.5 , 'MarkerSize', 5, 'DisplayName', 'Saída do demodulador');
hold off;
xlabel('Tempo (s)');
ylabel('Parte imaginária');
legend('Location', 'Best');

function espectro_frequencia_2(sinal, fs, legenda, estilo, varargin)
    [Pxx, f] = pwelch(sinal, hanning(round(length(sinal)/8)), [], 4096, fs, 'twosided');
    Pxx_dBm = 10 * log10(Pxx * 1000);  

    f = f / 1e6;

    plot(f - fs/(2*1e6), fftshift(Pxx_dBm), estilo, 'DisplayName', legenda, varargin{:}); 
